...
function updateDiagnostics(document, collection, tharosOutput) {
    collection.clear();
    ...
    for (const finding of result.findings) {
        ...
        const diagnostic = new vscode.Diagnostic(range, `${finding.message} [${finding.rule}]`, getSeverity(finding.severity));
        diagnostic.source = 'Tharos';
        diagnostic.code = finding.rule;
        // Use full finding as metadata (as much as possible)
        diagnostic.tharosFinding = finding;
        diagnostic.relatedInformation = [
            new vscode.DiagnosticRelatedInformation(new vscode.Location(document.uri, range), finding.explain || "No explanation provided.")
        ];
