...
The Go Core currently enforces the following local security policies:

| Category | Detection Rule | Description |
| :--- | :--- | :--- |
| **XSS Prevention** | `innerHTML`, `outerHTML` | Flagged as dangerousDOM sink. |
| **XSS Prevention** | `dangerouslySetInnerHTML` | React specific XSS warning. |
| **XSS Prevention** | `document.write` | Legacy XSS vector detection. |
| **Cryptography** | `Math.random()` | Insecure randomness detection. |
| **Cryptography** | `md5`, `sha1` | Weak hashing algorithm detection. |
| **Injection** | `SQL Injection` | Heuristic detection of concatenated SQL strings. |
